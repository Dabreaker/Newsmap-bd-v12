...
function toggleLang(){
  ...
  applyI18n();loadHome();loadExplore();
}
...
function applyTheme(){
  ...
  const tb=document.getElementById('theme-btn');if(tb)tb.textContent=THEME==='dark'?'🌙':'☀️';
}
function toggleTheme(){
  THEME=THEME==='dark'?'light':'dark';
  localStorage.setItem('jb_theme',THEME);applyTheme();
}
...
// ── GPS ────────────────────────────────────────────────────────
// GPS — returns null if user denied location
let _gpsGranted=false;
function gps(){
  return new Promise(res=>{
    if(S.userLat!==null)return res({lat:S.userLat,lon:S.userLon});
    if(!navigator.geolocation){
      // Device has no geolocation — allow with Dhaka default
      _gpsGranted=true;S.userLat=23.8103;S.userLon=90.4125;
      return res({lat:23.8103,lon:90.4125});
    }
    navigator.geolocation.getCurrentPosition(
      ...
      err=>{
        ...
        else{// timeout or unavailable — allow with last known or Dhaka
          _gpsGranted=true;if(!S.userLat){S.userLat=23.8103;S.userLon=90.4125;}
          res({lat:S.userLat,lon:S.userLon});
        }
      },
...
async function requireLocation(){
  const loc=await gps();
  if(!loc){
    toast('লগইন করতে লোকেশন অ্যাক্সেস দিন',true);
    return false;
  }
  return true;
}
...
// ══════════════════════════════════════════════════════════════
// MARKER MANAGER — geo-accurate 50×50m cells, scale with map zoom
// Stack per cell: img (bottom, non-interactive) → border rect (non-interactive) → hit rect (top, interactive)
// ══════════════════════════════════════════════════════════════
  ...
  _apply(data,zoom){
    ...
    filtered.forEach(n=>{
      ...
      if(!this.markers[n.id]){
        // 1. Visual layer — image fills cell or emoji fallback
        let img;
        if(n.thumb){
          img=L.imageOverlay(n.thumb,bounds,{opacity:1,interactive:false,crossOrigin:'anonymous',className:'nm-cell-img'}).addTo(MAP);
        } else {
          const icon=L.divIcon({html:`<div style="font-size:18px;text-align:center;line-height:1;pointer-events:none;">📰</div>`,iconSize:[24,24],iconAnchor:[12,12],className:''});
          img=L.marker([clat,clon],{icon,interactive:false,keyboard:false}).addTo(MAP);
        }
        // 2. Colored border rect — non-interactive
        const border=L.rectangle(bounds,{color:col,weight:2,opacity:0.9,fillColor:col,fillOpacity:n.thumb?0:0.15,interactive:false}).addTo(MAP);
        // 3. Transparent hit rect — top layer, captures all taps
        ...
        const nid=n.id;
        hit.on('click',function(e){L.DomEvent.stopPropagation(e);openModal(nid);});
...
function toggleMapFullscreen(){
  ...
  if(btn){btn.classList.toggle('active',isFs);btn.textContent=isFs?'⛶':'⛶';}
  setTimeout(()=>{if(MAP)MAP.invalidateSize();},80);
  // Exit fullscreen if user switches away from map tab
  if(isFs&&S.activeTab!=='map')document.body.classList.remove('map-fs');
}
...
function donationHTML(){return`<div class="donate-card glass"><div class="dc-head"><span style="font-size:22px">🤲</span><div><div style="font-weight:800;font-size:15px;color:var(--gold)">${t('help')}</div><div style="font-size:11px;color:var(--muted);margin-top:1px">জনবার্তা</div></div></div><p class="dc-msg">${t('help_msg')}</p><div class="dc-methods"><div class="dc-method" onclick="copyNum('bKash')"><div class="dc-logo" style="background:#E2136E">bK</div><div class="dc-info"><div class="dc-label">bKash</div><div class="dc-num">${DONATE_NUM}</div></div><div>📋</div></div><div class="dc-method" onclick="copyNum('Nagad')"><div class="dc-logo" style="background:#F7941D">Ng</div><div class="dc-info"><div class="dc-label">Nagad</div><div class="dc-num">${DONATE_NUM}</div></div><div>📋</div></div><div class="dc-method" onclick="copyNum('Rocket')"><div class="dc-logo" style="background:#8B1DB8">Rk</div><div class="dc-info"><div class="dc-label">Rocket</div><div class="dc-num">${DONATE_NUM}</div></div><div>📋</div></div></div><div class="dc-footer">সংবাদ হোক দালাল মুক্ত — জনবার্তা</div></div>`;}
...
async function loadNotifications(){
  ...
  if(!S.token){el.innerHTML=`<div class="empty"><p>${t('login_sub')}</p><button class="btn-p" style="margin-top:10px;max-width:200px" onclick="openAuth()">${t('enter')}</button></div>`;return;}
  el.innerHTML='<div class="sp-box"><b></b><b></b><b></b></div>';
...
// ══════════════════════════════════════════════════════════════
// MODAL — full bottom-sheet news detail
// ══════════════════════════════════════════════════════════════
...
function _renderNewsDetail(ct,n){
  ...
  // Escape title for use inside onclick attribute
  const safeTitle=n.title.replace(/\\/g,'\\\\').replace(/'/g,"\\'");
  ct.innerHTML=`
    ${imgs.length?`<div class="mcar">${imgs.map((s,i)=>`<div class="mslide"><div style="width:100%;aspect-ratio:1/1;border-radius:11px;overflow:hidden;"><img src="${esc(s)}" loading="lazy" style="width:100%;height:100%;object-fit:cover;display:block;" onerror="this.closest('.mslide').style.display='none'"></div>${imgs.length>1?`<div class="mcar-n">${i+1}/${imgs.length}</div>`:''}</div>`).join('')}</div>`:`<div class="noimgph">📷 ছবি নেই</div>`}
...
async function castVote(nid,type){
  if(!S.token){openAuth();return;}
  if(S.userLat==null){toast(t('enable_loc'),true);return;}
  const r=await api('POST','/api/vote',{news_id:nid,type,user_lat:S.userLat,user_lon:S.userLon});
  if(r.error){toast(r.error,true);return;}
  toast(`✓ (weight: ${r.weight})`);CACHE.invalidate();openModal(nid);
}
...
function openAuth(mode='login'){
  _authMode=mode;_setAuthMode(mode);
  document.getElementById('auth-overlay').classList.add('open');
  // Check location permission state and show warning if denied
  if(navigator.permissions){
    navigator.permissions.query({name:'geolocation'}).then(p=>{
      const warn=document.getElementById('auth-loc-warn');
      if(warn)warn.style.display=p.state==='denied'?'block':'none';
    }).catch(()=>{});
  }
}
async function retryLocation(){
  const warn=document.getElementById('auth-loc-warn');
  S.userLat=null;// force re-request
  const loc=await gps();
  if(loc&&warn)warn.style.display='none';
...
function _setAuthMode(mode){
  ...
  const btn=document.getElementById('auth-btn');if(btn)btn.textContent=isReg?t('register'):t('enter');
  const sub=document.getElementById('auth-mode-sub');if(sub)sub.textContent=isReg?t('register'):t('enter');
  const tog=document.getElementById('auth-toggle-text');
  if(tog)tog.innerHTML=isReg?`${t('have_account')} <a href="javascript:void(0)" onclick="_setAuthMode('login')" style="color:var(--accent);font-weight:700">${t('enter')}</a>`:`${t('no_account')} <a href="javascript:void(0)" onclick="_setAuthMode('register')" style="color:var(--accent);font-weight:700">${t('register')}</a>`;
}
async function doAuth(e){
  if(e)e.preventDefault();if(_authMode==='register'){doRegister();return;}
  ...
  btn.disabled=false;btn.textContent=t('enter');if(r.error){toast(r.error,true);return;}_authSuccess(r);
}
async function doRegister(){
  ...
  btn.disabled=false;btn.textContent=t('register');if(r.error){toast(r.error,true);return;}_authSuccess(r);
}
function _authSuccess(r){
  ...
  toast('✓ সফল');closeAuth();renderUser();if(S.admin)showAdminBtn();
...
async function loadAdmStats(){
  ...
  <div class="adm-stat glass" style="text-align:left;margin-bottom:12px;padding:14px 16px">
    <div class="adm-stat-lbl" style="margin-bottom:4px">💾 Storage</div>
    <div style="font-size:16px;font-weight:800">Vercel Blob</div>
    <div style="font-size:11px;color:var(--muted);margin-top:4px">${d.news_count||0} active news</div>
  </div>
...
async function loadAdmNews(q='',page=0){
  ...
  list.innerHTML+=(rows).map(n=>`<div class="adm-row">
    ...
    <div class="adm-row-info"><div class="adm-row-title">${esc(n.title)}</div><div class="adm-row-meta">uid:${n.owner_id} · ${(+n.real_score||0).toFixed(1)}✓ ${(+n.fake_score||0).toFixed(1)}✗ · ${n.vote_count||0}v</div></div>
